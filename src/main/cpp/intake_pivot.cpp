...
void Intake_pivot::run_intake_pivot ( int pivot_start, int pivot_high, int pivot_ball, int pivot_pickup, int hatch) {
if (mag_switch_claw->Get()==0){
	claw_pivot_talon_enc->SetSelectedSensorPosition(0,0,10);
}

claw_pivot_talon_enc->SetSensorPhase(1);
if (mode==0){
//Switching Pos
...
//Determining Ball Pos
		...
		if (current_pivot_pos ==4){
			claw_pivot_talon_enc->Set(ControlMode::Position, hatch);	
		}
		if (current_pivot_pos ==5){
		claw_pivot_talon_enc->Set(ControlMode::Position, pivot_pickup);	
		}
